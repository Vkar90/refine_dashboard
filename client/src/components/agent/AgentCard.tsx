...
const AgentCard = ({id, name, email, avatar, noOfProperties}:AgentCardProp) => {
  ...
  return (
    <Box 
      ...
      to={generateLink()}
      width="100%"
      sx={{ display: 'flex', flexDirection: {xs: 'column', sm: 'row'}, gap: '20px', padding: '20px', borderRadius: '10px',  '&hover': {boxShadow: '0 22px 45px 2px rgba(176,176,176,0.1)'}}}
    >
      ...
      <Stack direction="column" justifyContent='space-between' flex={1} gap={{xs:4, sm:2}}>
        <Stack gap={2} direction="row" flexWrap="wrap" alignItems="center">
          <Typography fontSize={20} fontWeight={600} color='#11142d'>{name}</Typography>
        ...
        <Stack direction="row" flexWrap='wrap' justifyContent='space-between' alignItems='center' gap={2}>
          ...
          <InfoBar icon={<Phone sx={{color: '#808191'}} />} name="+30 8545 659" />
